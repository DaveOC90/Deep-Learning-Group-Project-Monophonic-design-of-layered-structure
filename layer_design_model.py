...
def dense_forward(inputs, activation):
 ...
 #    dense6 = tf.layers.dense(dense5, dense6_size, activation=activation)
    # Output layer
    output = tf.layers.dense(dense4, len(output_labels), activation=activation)
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        print("Test Loss: {}".format(str(loss.eval(feed_dict={layer_inputs: test[input_labels].as_matrix(), layer_labels: test[output_labels].as_matrix()}))))
